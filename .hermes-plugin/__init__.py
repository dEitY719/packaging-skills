...
packaging skills are task-triggered (you reach for them when you are building
or auditing a marketplace repo), so there is nothing worth paying for on every
first turn.
"""
...
_SENTINEL = ("rename-repo", "SKILL.md")


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install dEitY719/packaging-skills`):
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, *_SENTINEL)):
            return cand
    raise RuntimeError(
        "packaging plugin: cannot find the skills/ tree "
