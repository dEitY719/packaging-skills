...
longer emits it** (dEitY719/dotfiles#1410 P-1). Rationale, the per-harness file
list and what CI gates: `references/options.md`. Reference repos:
`dEitY719/packaging-skills`, `dEitY719/harness-skills`.
...
missing, reject a `claude-plugin-` prefix, enforce lowercase-hyphen naming,
abort if `--src` missing or dest exists, infer skills from chat else ask.
...
`version` across all seven version-bearing manifests, `license` = `MIT`
...
Run `packaging:structure-check <dest>/<repo-name> --single`, confirm M1-M10
PASS, then emit the `[OK]` report from `references/help.md`.
